...
GlobePosition::GlobePosition(glm::vec2 position, float zoom, float radius, float movement_speed)
    : movement_speed(movement_speed), radius(radius), zoom(zoom), lat(position.y), lon(position.x)
{
}
