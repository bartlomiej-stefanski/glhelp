  ...
  vec4 matColor)
{
  ...
  float spec = pow(max(dot(viewDir, reflectDir), 0.0), 256);
  light += vec4(1.0) * spec * attenuation * intensity * specular * diffuseColor;
...
void main(void)
{
  ...
  vec4 matColor = vec4(0.01, 0.01, 0.01, 1.0);
  ...
  color = add_fog(max(length(cameraPos - fragPos) / 10, 1.0), color);
